	...
	<body>
		...
		<table id="menu">
			<thead class="thead-light">
				<tr>
					<th colspan="9" class="align-middle" style="text-align: center;">Periode {{$_GET['tanggalPinjam']!=null ?$_GET['tanggalPinjam']:'-'}} s/d {{$_GET['tanggalKembali']!=null?$_GET['tanggalKembali']:'-'}}</th>
				</tr>
				<tr>
					<th rowspan="2" class="align-middle" style="text-align: center;">Nama User</th>
					<th rowspan="2" class="align-middle" style="text-align: center;">Email</th>
					<th colspan="5" class="align-middle" style="text-align: center;">Buku</th>
			...
			<tbody>
				@foreach ($data['buku'] as $i => $item)
					@foreach ($item as $j => $detail)
					<tr>
						@if ($j == 0)
						<td rowspan="{{count($item)}}" style="vertical-align: middle;">{{$data['user'][$i]->nama}}</td>
						<td rowspan="{{count($item)}}" style="vertical-align: middle;">{{$data['user'][$i]->email}}</td>
						@endif
						<td>{{$detail->judul}}</td>
